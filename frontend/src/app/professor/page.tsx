    ...
    const syncBaseData = useCallback(async (showLoader = false, referenceDate?: string) => {
        ...
        try {
            ...
            const [tenantResponse, teacherResponse, notificationsResponse, calendarResponse, assessmentEventsResponse] = await Promise.all([
                fetch(`${API_BASE_URL}/tenants/current`, { headers: { } }),
                fetch(`${API_BASE_URL}/teachers/me`, { headers: { } }),
                fetch(`${API_BASE_URL}/notifications/my?status=ALL`, { headers: { } }),
                fetch(`${API_BASE_URL}/lesson-events/my-calendar?view=MONTH&referenceDate=${targetDate}`, { headers: { } }),
                fetch(`${API_BASE_URL}/lesson-assessments/my-events?status=ALL`, { headers: { } }),
            ]);
