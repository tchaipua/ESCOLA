...
export type GridSortState<ColumnKey extends string = string> = {
    column: ColumnKey;
...
export type GridColumnDefinition<RowType, ColumnKey extends string = string> = {
    ...
    getSortValue?: (row: RowType) => string | number;
...
export function normalizeGridPdfOptions(options?: Partial<GridPdfOptions> | null): GridPdfOptions {
    const lineClamp = options?.lineClamp;
    const normalizedLineClamp: GridPdfLineClamp =
        lineClamp === 0 || lineClamp === 1 || lineClamp === 2 || lineClamp === 3 || lineClamp === 4
            ? lineClamp
            : DEFAULT_GRID_PDF_OPTIONS.lineClamp;

    return {
        orientation: options?.orientation === 'portrait' || options?.orientation === 'landscape'
            ? options.orientation
            : DEFAULT_GRID_PDF_OPTIONS.orientation,
        rowDensity: options?.rowDensity === 'compact' || options?.rowDensity === 'comfortable' || options?.rowDensity === 'spacious'
            ? options.rowDensity
            : DEFAULT_GRID_PDF_OPTIONS.rowDensity,
        fontScale: options?.fontScale === 'small' || options?.fontScale === 'medium' || options?.fontScale === 'large'
            ? options.fontScale
            : DEFAULT_GRID_PDF_OPTIONS.fontScale,
        widthStrategy: options?.widthStrategy === 'compact' || options?.widthStrategy === 'balanced' || options?.widthStrategy === 'detailed'
            ? options.widthStrategy
            : DEFAULT_GRID_PDF_OPTIONS.widthStrategy,
        lineClamp: normalizedLineClamp,
        showColumnHeaders: typeof options?.showColumnHeaders === 'boolean'
            ? options.showColumnHeaders
            : DEFAULT_GRID_PDF_OPTIONS.showColumnHeaders,
        showRowStripes: typeof options?.showRowStripes === 'boolean'
...
async function blobToDataUrl(blob: Blob) {
    return new Promise<string>((resolve, reject) => {
        ...
        reader.onerror = () => reject(new Error('Não foi possível converter o logotipo para exportação.'));
...
function downloadFile(content: BlobPart, mimeType: string, extension: string, fileBaseName: string) {
...
export function buildExportColumnsFromGridColumns<RowType, GridKey extends string, ExtraKey extends string = never>(
    gridColumns: Array<GridColumnDefinition<RowType, GridKey>>,
    extraColumns: Array<GridColumnDefinition<RowType, GridKey | ExtraKey>> = [],
) {
    const mergedColumns = new Map<string, GridColumnDefinition<RowType, GridKey | ExtraKey>>();

    gridColumns.forEach((column) => {
        mergedColumns.set(column.key, column as GridColumnDefinition<RowType, GridKey | ExtraKey>);
    });
    ...
    return Array.from(mergedColumns.values()) as GridColumnDefinition<RowType, GridKey | ExtraKey>[];
}

export function sortGridRows<RowType, ColumnKey extends string>(
    rows: RowType[],
    columns: GridColumnDefinition<RowType, ColumnKey>[],
    sortState: GridSortState<ColumnKey>,
    fallbackSort?: (left: RowType, right: RowType) => number,
) {
    const currentColumn = columns.find((column) => column.key === sortState.column);
    if (!currentColumn) {
        return [...rows];
    }

    return [...rows].sort((left, right) => {
        const leftValue = currentColumn.getSortValue ? currentColumn.getSortValue(left) : currentColumn.getValue(left);
        const rightValue = currentColumn.getSortValue ? currentColumn.getSortValue(right) : currentColumn.getValue(right);

        if (leftValue < rightValue) return sortState.direction === 'asc' ? -1 : 1;
        if (leftValue > rightValue) return sortState.direction === 'asc' ? 1 : -1;

        return fallbackSort ? fallbackSort(left, right) : 0;
    });
}
...
export async function exportGridRows<RowType, ColumnKey extends string>(options: {
    ...
    const branding = {
        ...options.branding,
        schoolName: options.branding?.schoolName || cachedBranding?.schoolName,
        logoUrl: options.branding?.logoUrl || cachedBranding?.logoUrl || null,
    };
    const pdfOptions = normalizeGridPdfOptions(options.pdfOptions);
    const activeColumns = options.columns.filter((column) => options.selectedColumns[column.key]);
    ...
    const rows = options.rows.map((row) => activeColumns.map((column) => column.getValue(row)));
    ...
    if (options.format === 'json') {
        downloadFile(JSON.stringify(exportRows, null, 2), 'application/json;charset=utf-8;', 'json', options.fileBaseName);
        return;
    }

    const tableHtml = `
        <table border="1" cellspacing="0" cellpadding="8" class="grid-export-table">
            <thead>
                <tr>${headers.map((header) => `<th style="background:#e2e8f0;">${escapeHtmlValue(header)}</th>`).join('')}</tr>
            </thead>
            <tbody>
                ${rows.map((row) => `<tr>${row.map((value) => `<td><div class="cell-content">${escapeHtmlValue(value)}</div></td>`).join('')}</tr>`).join('')}
            </tbody>
        </table>
    `;
    ...
    if (options.format === 'excel') {
        ...
        const exportSubtitle = branding.subtitle || 'Exportação com os filtros atualmente aplicados.';
        ...
        worksheet.getCell('B1').value = schoolName || 'ESCOLA';
        ...
        activeColumns.forEach((column, columnIndex) => {
            ...
            cell.alignment = { vertical: 'middle', horizontal: column.align === 'right' ? 'right' : column.align === 'center' ? 'center' : 'left' };
            cell.border = {
        ...
        rows.forEach((rowValues, rowIndex) => {
            ...
            rowValues.forEach((value, columnIndex) => {
                ...
                cell.alignment = {
                    vertical: 'middle',
                    horizontal: activeColumns[columnIndex]?.align === 'right'
                        ? 'right'
        ...
        downloadFile(buffer, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'xlsx', options.fileBaseName);
        return;
    }

    const printWindow = window.open('', '_blank', 'width=1100,height=800');
    if (!printWindow) {
        throw new Error('Não foi possível abrir a visualização para PDF. Verifique se o navegador bloqueou a janela.');
    }

    const schoolName = branding.schoolName || '';
    const logoMarkup = branding.logoUrl
        ? `<img src="${escapeHtmlValue(branding.logoUrl)}" alt="${escapeHtmlValue(`Logo de ${schoolName || 'escola'}`)}" style="width:84px;height:84px;object-fit:contain;border:1px solid #cbd5e1;border-radius:16px;background:#ffffff;padding:8px;" />`
        : '';

    const headerMarkup = schoolName || branding.title
        ? `
    ...
    const fontSize = pdfOptions.fontScale === 'small'
        ? '10px'
        : pdfOptions.fontScale === 'large'
            ? '12px'
            : '11px';
    const headerFontSize = pdfOptions.fontScale === 'small'
        ? '10px'
        : pdfOptions.fontScale === 'large'
            ? '12px'
            : '11px';
    const cellPadding = pdfOptions.rowDensity === 'compact'
        ? '5px 6px'
        : pdfOptions.rowDensity === 'spacious'
            ? '10px 11px'
            : '8px 9px';
    const lineClampCss = pdfOptions.lineClamp === 0
    ...
    const rowStripeCss = pdfOptions.showRowStripes
        ...
        : '';
    const widthStrategyCss = pdfOptions.widthStrategy === 'compact'
    ...
    const headerlessColumnsPerRow = pdfOptions.orientation === 'portrait'
        ? 2
        : pdfOptions.widthStrategy === 'compact'
            ...
            : pdfOptions.widthStrategy === 'detailed'
                ? 2
                : 3;
    const headerlessLabelWidth = headerlessColumnsPerRow >= 4
        ? '96px'
    ...
    const getHeaderlessFieldSpan = (label: string, value: string) => {
        ...
        const needsExtraSpace = normalizedValue.length > (headerlessColumnsPerRow >= 4 ? 42 : 34) || label.length > 18 || normalizedValue.includes('\n');
