...
export default function CalendarioAulasPage() {
    ...
    const handleSaveEvent = async () => {
        ...
        try {
            ...
            setStatusMessage(modalState.existingEvent ? 'Evento da aula atualizado com sucesso.' : 'Evento da aula lançado com sucesso.');
            setModalState(null);
                                            ...
                                            >
                                                ...
                                                <div className="mt-4 space-y-2">
                                                    {dayItems.length ? (
                                                        ...
                                                        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-3 py-5 text-center text-xs font-bold uppercase tracking-[0.14em] text-slate-400">
                                                            Sem aula
                                                        </div>
                                                    )}
                                                </div>
                        ...
                        <div className="mt-5 space-y-4">
                            {selectedDayItems.length ? (
                                            ...
                                            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                                                {item.events.map((event) => (
                                                    <div key={event.id} className={`rounded-2xl border px-4 py-3 ${getEventTone(event.eventType)}`}>
                                                        ...
                                                        {event.eventType === 'PROVA' || event.eventType === 'TRABALHO' ? (
                                                            <button
                                                                type="button"
                                ...
                                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-10 text-center text-sm font-medium text-slate-500">
                                    Não há aulas para a data selecionada. Clique em outro dia do calendário para navegar pela sua grade anual.
                                </div>
                            )}
                        </div>
                    </div>
