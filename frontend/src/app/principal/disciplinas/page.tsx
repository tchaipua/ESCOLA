...
export default function DisciplinasPage() {
    ...
    const subjectFilterOptions = useMemo(
        () => Array.from(new Set(subjects.map((subject) => subject.name).filter((name): name is string => Boolean(name && name.trim()))))
    ...
    const renderSubjectColumnHeader = (column: ConfigurableGridColumn<Subject, SubjectColumnKey>) => {
        ...
        const isOpen = activeSubjectFilterColumn === column.key;
        const isActive = Boolean(subjectColumnFilters[column.key]?.trim()) || sortState.column === column.key;
        const filterValue = subjectColumnFilterDrafts[column.key] || '';
        ...
        return (
            <div className={`relative flex items-center gap-2 ${column.align === 'right' ? 'justify-end' : column.align === 'center' ? 'justify-center' : 'justify-start'}`}>
                ...
                <button
                    type="button"
                    onClick={() => openSubjectColumnFilter(isOpen ? null : column.key)}
                ...
                {isOpen ? (
                    <div className={`absolute left-0 top-full z-40 mt-2 w-[292px] rounded-2xl border border-slate-200 bg-white p-3 text-left shadow-xl ${column.align === 'right' ? 'right-0 left-auto' : ''}`}>
                        ...
                        <div className="mt-3 border-t border-slate-100 pt-3">
                            <div className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Filtrar disciplina</div>
                            <select
                                value={filterValue}
                                onChange={(event) => setSubjectColumnFilterDrafts((current) => ({
                                    ...current,
                            ...
                            >
                                ...
                                {subjectFilterOptions.map((subjectName) => (
                                    <option key={subjectName} value={subjectName}>{subjectName}</option>
                                ))}
                            </select>
                            <button
                                type="button"
                                onClick={() => applySubjectColumnFilter(column.key)}
                            ...
                            <button
                                type="button"
                                onClick={() => clearSubjectColumnFilter(column.key)}
