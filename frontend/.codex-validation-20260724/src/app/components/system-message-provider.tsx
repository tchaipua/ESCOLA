...
export default function SystemMessageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const wrappedFetch: typeof window.fetch = async (input, init) => {
      ...
      const messageMode = new Headers(init?.headers).get("x-msinfor-system-message");
      const silentMessage = messageMode?.toLowerCase() === "silent";
      ...
      try {
        ...
        if (userInitiated && !silentMessage) {
          display({
            type: "error",
      ...
      if (!userInitiated || silentMessage) return response;
  ...
  useEffect(() => {
    ...
    const capture = (element: HTMLElement) => {
      ...
      if (element.closest("[data-system-message-ignore]")) return;
